...
type server struct {
	...
	tokenCounter       int64
	contadorIDPaciente int64 //NUEVO
	contadorIDMedico   int64 //NUEVO
}
...
func (s *server) registerUser(req api.Request) api.Response {
	// Validación básica
	if req.Username == "" || req.Password == "" || req.Apellido == "" {
...
func (s *server) obtenerHistoriales(req api.Request) api.Response {
	if req.Username == "" || req.Token == "" {
		return api.Response{Success: false, Message: "Faltan credenciales"}
	}
	if !s.isTokenValid(req.Username, req.Token) {
	...
	historiales, err := s.db.Get("historiales", []byte(req.Username)) //GET PARA OBTENER LOS DATOS DE LOS PACIENTES DE UN MÉDICO
	if err != nil {
		return api.Response{Success: false, Message: "Error al actualizar datos del usuario"}
	}
	return api.Response{Success: True, Message: "Expedientes de la especialidad" + "CONSEGUIR LA ESPCIALIDAD", Expedientes: lista_expedientes} //lista de los expedientes de la especialidad del médico
}
